...
setproctitle.setproctitle("train_stdc_camvid_msc_fapn_zerorains")
...
def parse_args():
    ...
    parse.add_argument(
        ...
        dest='respath',
        type=str,
        default="checkpoints/camvid_msc_fapn_optim_STDC2-Seg/",
    )
    # 主干网络
    parse.add_argument(
        '--backbone',
...
def train():
    ...
    diter = iter(dl)
    epoch = 0
    tensor_board_path = os.path.join("./logs", "camvid_msc_fapn" + '{}'.format(time.strftime('%Y-%m-%d-%H-%M-%S')))
