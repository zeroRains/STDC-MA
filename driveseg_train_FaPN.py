...
from driveseg import DriveSeg
from loss.loss import OhemCELoss, CriterionOhemDSN
...
setproctitle.setproctitle("train_fapn_driveseg_zerorains")
...
def parse_args():
    ...
    parse.add_argument(
        ...
        dest='respath',
        type=str,
        default="checkpoints/driveseg_FaPN_optim_STDC2-Seg/",
    )
    # 主干网络
    parse.add_argument(
        '--backbone',
...
def train():
    ...
    # 加载数据集
    ds = DriveSeg(dspth, cropsize=cropsize, mode=mode, randomscale=randomscale)
    ...
    dl = DataLoader(ds,
                    ...
                    drop_last=True)
    # exit(0)
    dsval = DriveSeg(dspth, mode='test', randomscale=randomscale)
    ...
    tensor_board_log = os.path.join(args.respath, "logs")
    # if os.path.exists(tensor_board_log):
    #     os.mkdir(tensor_board_log)
    tensor_board_path = os.path.join("./logs", "driveseg_fapn_" + '{}'.format(time.strftime('%Y-%m-%d-%H-%M-%S')))
    ...
    for it in range(max_iter):
        ...
        if (it + 1) % save_iter_sep == 0:  # and it != 0:
            ...
            logger.info('mIOU75 is: {}, mIOU is: {}'.format(mIOU50, mIOU75))
            # exit(0)
            # exit(1)
            if mIOU50 > maxmIOU50:
                maxmIOU50 = mIOU50
            ...
            visual.add_scalar("MIOU75", mIOU50, (it + 1) // save_iter_sep)
            logger.info('maxmIOU75 is: {}, maxmIOU is: {}.'.format(maxmIOU50, maxmIOU75))
