...
# model = BiSeNet(backbone='STDCNet1446', n_classes=19, pretrain_model=None,use_boundary_2=False, use_boundary_4=False, use_boundary_8=True,use_boundary_16=False, use_conv_last=False)
## BisNet的上下文路径
...
class BiSeNet(nn.Module):
    def __init__(self, backbone, n_classes, pretrain_model='', use_boundary_2=False, use_boundary_4=False,
        ...
        self.aux_head = nn.Sequential(
            nn.Conv2d(3 * n_classes, 3 * n_classes, kernel_size=1, stride=1, padding=0),
            nn.BatchNorm2d(3 * n_classes),
            nn.ReLU(),
            nn.Conv2d(3 * n_classes, n_classes, kernel_size=1, stride=1, padding=0, bias=True)
        )
        self.conv3x3_ocr = nn.Sequential(
            nn.Conv2d(3 * n_classes, 512, kernel_size=3, stride=1, padding=1),
            nn.BatchNorm2d(512),
            nn.ReLU()
        )
        self.ocr_gather_head = SpatialGatherModule(n_classes)
        self.ocr_distri_head = SpatialOCRModule(in_channels=512,
                                                key_channels=256,
                                                out_channels=512,
                                                scale=1,
                                                dropout=0.05, )
        self.cls_head = nn.Conv2d(512, n_classes, kernel_size=1, stride=1, padding=0, bias=True)
    ...
    def forward(self, x):
        ...
        feat_out = torch.cat([feat_out, feat_out16, feat_out32], 1)

        # 只对feat_out做OCR
        out_aux = self.aux_head(feat_out)
        feat_out = self.conv3x3_ocr(feat_out)

        context = self.ocr_gather_head(feat_out, out_aux)
        feat_out = self.ocr_distri_head(feat_out, context)

        feat_out = self.cls_head(feat_out)
        ...
        if (not self.use_boundary_2) and (not self.use_boundary_4) and self.use_boundary_8:
            return feat_out, feat_out16, feat_out32, feat_out_sp8, out_aux
        # 除了输出结果其他都不要
