...
from modules.DCNv2.dcn_v2 import DCN as dcn_v2
...
class FeatureAlign_V2(nn.Module):  # FaPN full version
    ...
    def forward(self, feat_l, feat_s, main_path=None):
        # 前一个是分辨率大的低级特征(encoder阶段的特征图)
        # 后一个是分辨率小的高级特征(decoder阶段的特征图)
        HW = feat_l.size()[2:]
        if feat_l.size()[2:] != feat_s.size()[2:]:
            feat_up = F.interpolate(feat_s, HW, mode='bilinear', align_corners=False)
        else:
            feat_up = feat_s
        # low_level = feat_l.clone().detach().cpu()
        # low_level *= 255.0
        # save_image_tensor2cv2(low_level, "1_low_level.jpg")
        # high_level = feat_up.clone().detach().cpu()
        # utils.save_image(high_level * 2, "1_high_level.jpg")
        # 特征选择
        feat_arm = self.lateral_conv(feat_l)  # 0~1 * feats
        # result_fcm = feat_arm.clone().detach().cpu()
        # utils.save_image(result_fcm, "1_result_in_fcm.jpg")
        # 对选择特征和的结果(低级特征)与高级特征在通道维度上进行拼接
        offset = self.offset(torch.cat([feat_arm, feat_up * 2], dim=1))  # concat for offset by compute the dif
        # 使用课变形卷积将偏移量与高级特征拼接的结果进行融合，然后relu激活
        feat_align = self.relu(self.dcpack_L2([feat_up, offset], main_path))  # [feat, offset]
        # result = feat_align + feat_arm
        # result = result.clone().detach().cpu()
        # utils.save_image(result, "1_result_in_fam.jpg")
        # 经典残差
...
class ContextPath(nn.Module):
    def __init__(self, backbone='CatNetSmall', pretrain_model='', use_conv_last=False, *args, **kwargs):
        ...
        if backbone == 'STDCNet1446':
            ...
            self.arm16 = FeatureAlign_V2(512, 128)
            inplanes = 1024
            if use_conv_last:
                inplanes = 1024
            # self.arm32 = AttentionRefinementModule(inplanes, 128)
            self.arm32 = FeatureAlign_V2(inplanes, 128)
...
class BiSeNet(nn.Module):
    ...
    def _fwd(self, x):
        ...
        # 这里就是多尺度注意力时间了，这里输入的是ocr(两个3x3卷积，一个1x1卷积，维度没有改变)，最后一层用sigmoid
