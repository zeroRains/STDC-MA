...
setproctitle.setproctitle("train_fapn_stdc_sunrgbd_zerorains")
...
def parse_args():
    ...
    parse.add_argument(
        ...
        dest='respath',
        type=str,
        default="checkpoints/sunrbgd_FaPN_optim_STDC2-Seg/",
    )
    # 主干网络
    parse.add_argument(
        '--backbone',
...
def train():
    ...
    # 加载数据集
    ds = SunRGBD(dspth, cropsize=cropsize, mode=mode)
    ...
    net.load_state_dict(torch.load("/home/disk2/ray/workspace/zerorains/stdc/STDC2optim_sun_rgbd_fapn.pth",map_location='cpu'))
    net.cuda(CUDA_ID)
    net.train()
    ...
    tensor_board_path = os.path.join("./logs", "sunrgbd_fapn_" + '{}'.format(time.strftime('%Y-%m-%d-%H-%M-%S')))
