...
setproctitle.setproctitle("train_msc_rmi_stdc_sunrgbd_zerorains")
...
def parse_args():
    ...
    parse.add_argument(
        ...
        dest='respath',
        type=str,
        default="checkpoints/sunrgbd_MSC_RMI_optim_STDC2-Seg/",
    )
    # 主干网络
    parse.add_argument(
        '--backbone',
...
def train():
    ...
    diter = iter(dl)
    epoch = 0
    tensor_board_path = os.path.join("./logs", "sunrgbd_msc_" + '{}'.format(time.strftime('%Y-%m-%d-%H-%M-%S')))
