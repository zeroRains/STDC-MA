...
setproctitle.setproctitle("train_msc_fapn_rmi_sunrgbd_stdc_zerorains")
...
def parse_args():
    ...
    parse.add_argument(
        ...
        dest='respath',
        type=str,
        default="checkpoints/sunrgbd_MSC_FaPN_RMI_optim_STDC2-Seg/",
    )
    # 主干网络
    parse.add_argument(
        '--backbone',
...
def train():
    ...
    net.load_state_dict(
        torch.load("/home/disk2/ray/workspace/zerorains/stdc/STDC2optim_sun_rgbd_msc_fapn.pth", map_location='cpu'))
    net.cuda()
    ...
    tensor_board_path = os.path.join("./logs", "sunrgbd_msc_fapn_" + '{}'.format(time.strftime('%Y-%m-%d-%H-%M-%S')))
