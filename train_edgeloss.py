...
from loss.loss import SemanticEdgeLoss
...
def parse_args():
    ...
    parse.add_argument(
        ...
        dest='respath',
        type=str,
        default="checkpoints/edge_optim_STDC2-Seg/",
    )
    # 主干网络
    parse.add_argument(
        '--backbone',
...
def train():
    ...
    net = torch.load("./checkpoints/edge_optim_STDC2-Seg/pths/model_maxmIOU75.pth", map_location="cuda:3")
    # print(net)
    # exit(0)
    net.cuda(CUDA_ID)
    net.train()
    ...
    semantic_loss = SemanticEdgeLoss()
    # 细节损失
    ...
    #     max_iter=max_iter,
    #     power=power)
    optim = torch.optim.Adam(net.parameters(), lr=0.0002, betas=(0.9, 0.999),
                             eps=1e-08,
                             weight_decay=0)
    ## train loop
    msg_iter = 20
    ...
    for it in range(max_iter):
        ...
        edge_loss = semantic_loss(out, lb)
        ...
        loss = lossp + loss2 + loss3 + boundery_bce_loss + boundery_dice_loss + edge_loss
        # print(type(loss))
        loss.backward()
        optim.step()

        loss_avg.append(loss.item())
        loss_edge.append(edge_loss.item())
        ...
        if (it + 1) % msg_iter == 0:
            ...
            loss_edge = sum(loss_edge) / len(loss_edge)
            # lr = optim.lr
