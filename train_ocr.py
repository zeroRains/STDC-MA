...
from models.model_stages_OCR import BiSeNet
...
def parse_args():
    ...
    parse.add_argument(
        ...
        dest='respath',
        type=str,
        default="checkpoints/OCR_optim_STDC2-Seg/",
    )
    # 主干网络
    parse.add_argument(
        '--backbone',
...
def train():
    ...
    net = torch.load("./checkpoints/OCR_optim_STDC2-Seg/pths/model_maxmIOU75.pth", map_location="cuda:3")
    # print(net)
    # exit(0)
    net.cuda(CUDA_ID)
    net.train()
    ...
    criteria_aux = nn.CrossEntropyLoss(ignore_index=ignore_idx)
    # 细节损失
    ...
    for it in range(max_iter):
        ...
        if (not use_boundary_2) and (not use_boundary_4) and use_boundary_8:
            out, out16, out32, detail8, out_aux = net(im)
        ...
        loss_aux = criteria_aux(out_aux, lb)
        ...
        loss = lossp + loss2 + loss3 + loss_aux + boundery_bce_loss + boundery_dice_loss

        visual.add_scalar("total-loss", loss, it)
        visual.add_scalar("aux-loss", lossp, it)
        visual.add_scalar("ocr-loss", loss_aux, it)
